...
const int DATA = A0;
const int CLOCK = 5;	//old: 2
const int LOAD = 7;		//old: 3
const int RESET = 8; 	//old: 4
...
void TrackReporterS88::refresh() {
	...
	actualVal = digitalRead(DATA) ? 1 : 0;
	if(bitRead(mSwitches[myByte], myBit) != actualVal)
	{
		Serial.print("FB:0:");
		Serial.println(actualVal);
	}
	bitWrite(mSwitches[myByte], myBit++, actualVal);
